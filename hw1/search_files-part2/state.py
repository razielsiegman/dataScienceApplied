...
def hdistance2(s): #This will be the Manhattan distance heuristic
    total_distance = 0
    for i, j in enumerate(s[0]):
        #calculate how many positions the given tile must move vertically
        vertical = abs((j/3) - (i/3))
        #calculate how many positions the given tile must move horizontally
        horizontal = abs((j%3) - (i%3))
